...
- 백 버퍼와 프론트 버퍼를 교체하여 디스플레이에 임지를 보여주는데 이를 '스왑'이라고 한다
...
SwapChain
- 가상 프레임 버퍼의 집합
- 그래픽 카드와 그래픽 API가 프레임 속도를 안정화하고 화면 찢어짐을 줄이는 등의 목적을 위해 사용
- 주로 더블 버퍼링이나 트리플 버퍼링 같은 기법을 사용하여 화면에 렌더링된 프레임을 교체
- 화면 깜빡임이나 눈부심을 방지
...
RenderTargetView
- 렌더링 파이프라인에서 출력된 픽셀 정보를 저장하는 뷰
- 뷰는 리소스의 특정 부분에 대한 접근 방식을 정의하는 인터페이스
- 임시 중간 버퍼에 장면을 렌더링할 수 있게 해준다
- 이를 통해 더 복잡하거나 효과적인 렌더링을 수행할 수 있다
